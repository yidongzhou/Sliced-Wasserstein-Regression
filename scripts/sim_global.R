...
## - Sets swwr cores = 1 to avoid nested parallelism
...
run_one_mc <- function(run_id, d, N, n, xOut, mu_true, Sigma_true) {
  ...
  # Run SWWR regression on original data
  res_swwr <- swwr(
    y = y,
    x = x,
    xOut = xOut,
    optns = list(method = "global", verbose = FALSE, cores = 1)
  )
  
  # Run SWWR regression on transported data
  res_swwr_trans <- swwr(
    y = yT,
    x = x,
    xOut = xOut,
  ...
  # Run SAWR regression on original data
  res_sawr <- sawr(
    y = y,
    x = x,
    xOut = xOut,
  ...
  # Run SAWR regression on transported data
  res_sawr_trans <- sawr(
    y = yT,
    x = x,
    xOut = xOut,
...
for (d in dims) {
  ...
  out_file <- swr_path("data", sprintf("swr_global_d%d.rds", d))
...
cat("\n=== SIMULATION SUMMARY ===\n")
for (d in dims) {
  f <- swr_path("data", sprintf("swr_global_d%d.rds", d))
  if (file.exists(f)) {
    res <- readRDS(f)
    cat(sprintf("\nDimension d=%d:\n", d))
    for (nm in names(res)) {
